...
void CG_AddPlayerWeapon(refEntity_t *parent, playerState_t *ps, centity_t *cent)
{
	...
	qboolean        shouldDrawMuzzleFlashDlight = ((shouldDrawMuzzleFlash && cg_muzzleFlashDlight.integer == -1) || (cg_muzzleFlashDlight.integer > 0 && !(cg_muzzleFlashDlight.integer == 2 && ps && isFirstPerson)));
...
void CG_AddViewWeapon(playerState_t *ps)
{
	...
	if (cg_fov.value > 75)
	{
		fovOffset = -0.2f * (cg_fov.value - cg_gunFov.value);
	}
	else
	{
		fovOffset = 0;
	...
	if (ps->eFlags & EF_MOUNTEDTANK)
	{
		...
		if (cg.time - cg.predictedPlayerEntity.firedTime < MUZZLE_FLASH_TIME)
		{
			...
			if ((cg_muzzleFlashDlight.integer > 0) || (cg_muzzleFlash.integer == 1 && cg_muzzleFlashDlight.integer == -1))
			{
				trap_R_AddLightToScene(flash.origin, 320, 1.25f + (rand() & 31) / 128.0f, 1.0f, 0.6f, 0.23f, 0, 0);
			}
		}
