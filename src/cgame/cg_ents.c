...
static qboolean CG_LerpRifleGrenadeDir(centity_t *cent, vec3_t outDir)
{
	...
	if (cent->rifleGrenadeLaunchTime != cent->currentState.pos.trTime)
	{
		cent->rifleGrenadeLaunchTime = cent->currentState.pos.trTime;
		cent->rifleGrenadeLerpTime   = cg.time;
		if (speed > 0.f)
		{
			VectorCopy(targetDir, cent->rifleGrenadeDir);
		}
	}

	if (speed <= 0.f)
	{
		if (VectorNormalize2(cent->rifleGrenadeDir, outDir) > 0.f)
		{
			return qtrue;
		}
	...
	elapsed = cg.time - cent->rifleGrenadeLerpTime;
	if (elapsed < 0)
	{
		cent->rifleGrenadeLerpTime = cg.time;
		VectorCopy(cent->rifleGrenadeDir, outDir);
		return qtrue;
	}
	...
	while (steps-- > 0)
	{
		VectorScale(cent->rifleGrenadeDir, 1.0f - stepFrac, blended);
		VectorMA(blended, stepFrac, targetDir, blended);
		if (VectorNormalize2(blended, cent->rifleGrenadeDir) == 0.f)
		{
			VectorCopy(targetDir, cent->rifleGrenadeDir);
			break;
		}
